...
- Know what each write *accepts* before sending it. `update_workflow` mutates only `name`, `description`, `goal_statement`, `is_paused`, and `master_workflow_id`; every call must carry at least one of them. `language`, `timezone`, `channels`, `agent_name`, and other identity fields are set at `create_workflow` and are **not** `update_workflow` parameters — routing them through it (or calling it with only a `workflow_id`) fails `INVALID_INPUT`. Change those via the correct tool or dashboard, not an empty `update_workflow`.
