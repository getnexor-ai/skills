...
| 8 | Preflight | `review_agent_system_plan({ plan })` with `meeting_type`, `hosts[]` and `calendar_plan` on the appointment agent | Steps 1–7 | `ready_for_signoff: true`, then the operator approves the summarized plan ("Does this look ok? Let me know and I'll build it."; never show the `plan_fingerprint`) |
