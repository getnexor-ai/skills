...
The platform ships its own required process, returned by `describe_agent_configuration`. It is not optional and it is not advice — `review_agent_system_plan` returns a `signoff_prompt` that explicitly instructs you not to mutate anything until the user approves the summarized plan.
...
3. **`review_agent_system_plan({ plan })`** — preflight the whole system before mutating. It validates the plan and returns `ready_for_signoff`, `blocking_issues`, `clarification_questions`, and an internal `plan_fingerprint` (never shown to the user).
4. **Get explicit user approval of the plan.** Show the returned summary verbatim and close with "Does this look ok? Let me know and I'll build it." Never show the `plan_fingerprint`, any hash, or the word "fingerprint" to the user.
...
| Agent-group membership | `update_workflow({ master_workflow_id })` moves the mode into another agent; `null` is refused — a mode always belongs to an agent | `list_agent_groups`, `list_workflows` |
...
| Agent-group create/rename/delete | `create_agent_group` (the FIRST write of any build: `create_workflow` requires its id as `master_workflow_id`), `update_agent_group`, `delete_agent_group` | `list_agent_groups`, `list_workflows` |
...
| Create a mode (always INSIDE an agent: `master_workflow_id` required — `create_agent_group` first, or an existing agent's id) + skeleton statuses + fields | `create_workflow` | `get_workflow`, `list_agent_groups` |
...
| `update_workflow.master_workflow_id` | Relationship replacement; UUID moves/assigns, `null` is refused (no mode without an agent) | Resolve the group with `list_agent_groups`; read back `list_workflows` |
